...
public class EditorUtil : MonoBehaviour
{
    ...
    #region Lable Draw
    public static void DrawLabel(string label, bool padding, params GUILayoutOption[] options)
    {
    ...
    #region 직열화된 변수 Draw
    //SerializedObject를 받아서 SerializedProperty를 찾아서 보여줌.
    static public SerializedProperty DrawProperty(string label, SerializedObject serializedObject, string property, bool padding, params GUILayoutOption[] options)
    {
        ...
        if (sp != null)
        {
            ...
            if (label != null) EditorGUILayout.PropertyField(sp, new GUIContent(label/*, tip*/), options);
    ...
    #region 일반변수 Draw
    //Field타입에만 사용.. property는 만들어야한다.
    public static void DrawVariable_Field<T>(string label, T obj, string field, bool padding, params GUILayoutOption[] options)
    {
        FieldInfo f_info = obj.GetType().GetField(field);
        ...
        System.Type type = f_info.FieldType;

        if (type == typeof(int))
        {
            f_info.SetValue(obj, EditorGUILayout.IntField(new GUIContent(label), (int)f_info.GetValue(obj), options));
        }
        else if (type == typeof(long))
        {
            f_info.SetValue(obj, EditorGUILayout.LongField(new GUIContent(label), (long)f_info.GetValue(obj), options));
        }
        else if (type == typeof(float))
        {
            f_info.SetValue(obj, EditorGUILayout.FloatField(new GUIContent(label), (float)f_info.GetValue(obj), options));
        }
        else if (type == typeof(bool))
        {
            f_info.SetValue(obj, EditorGUILayout.Toggle(new GUIContent(label), (bool)f_info.GetValue(obj), options));
        }
        else if (type == typeof(string))
        {
            f_info.SetValue(obj, EditorGUILayout.TextField(new GUIContent(label), (string)f_info.GetValue(obj), options));
        }
        else if (type == typeof(Vector2))
        {
            f_info.SetValue(obj, EditorGUILayout.Vector2Field(new GUIContent(label), (Vector2)f_info.GetValue(obj), options));
        }
        else if (type == typeof(Vector3))
        {
            f_info.SetValue(obj, EditorGUILayout.Vector3Field(new GUIContent(label), (Vector3)f_info.GetValue(obj), options));
        }
        else if (type.IsEnum)
        {
            f_info.SetValue(obj, EditorGUILayout.EnumPopup(new GUIContent(label), (System.Enum)f_info.GetValue(obj), options));
        }
    ...
    //어떻게든 자동화 시킬려고 DrawVariable_Field함수를 만들었는데 배열은... 아 
    //일단 급하니까 다른방법 찾을시간 없고 각개전투할수 있도록 각 형식자별 함수 추가한다.
    public static int DrawVariable_Int(string label, int value, bool padding, params GUILayoutOption[] options)
    {
        return EditorGUILayout.IntField(new GUIContent(label), value, options);
    }
    public static bool DrawVariable_Bool(string label, bool value, bool padding, params GUILayoutOption[] options)
    {
        return EditorGUILayout.Toggle(new GUIContent(label), value, options);
    }
    public static System.Enum DrawVariable_Enum(string label, System.Enum value, bool padding, params GUILayoutOption[] options)
    {
        return EditorGUILayout.EnumPopup(new GUIContent(label), value, options);
    }
    ...
    static public bool DrawButton_Click(string text, params GUILayoutOption[] options)
    {
        return GUILayout.Button(text, options);//GUILayout.Width(76f)
    }


    static public bool DrawButton_DropDown(string text, params GUILayoutOption[] options)
    {
        return GUILayout.Button(text, "DropDown", options/*GUILayout.Width(76f)*/);
    }
    ...
    static public Color32[] AddBorder(Color32[] colors, int width, int height)
    {
        ...
        {
            int y1 = 0;//NGUIMath.ClampIndex(y2 - 1, height); //이부분 수정하면 사용가능할듯한데..시간있으면.

            for (int x2 = 0; x2 < w2; ++x2)
            {
                int x1 = 0;//NGUIMath.ClampIndex(x2 - 1, width); //이부분 수정하면 사용가능할듯한데..시간있으면.
                int i2 = x2 + y2 * w2;
    ...
    //이미지를 Inspector에 그리는거 같은데 시간나면 보자!!
